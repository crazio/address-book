...
module.exports = srv => {
    ...
    srv.on(events.SAVE_EVENT, AddressBookService.entities.Persons.name, async (req, next) => {
        const data = utils.getWithoutVirtualFields(req.data, ["fullName"]);
        return INSERT.into(Persons.cdsName).entries(data);
    });
